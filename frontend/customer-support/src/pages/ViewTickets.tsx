...
const ViewTickets: React.FC = () => {
  ...
  const fetchTickets = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_DOMAIN}/tickets`, {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_DOMAIN}/tickets/search?query=${searchQuery}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
  ...
  const openTicketDetails = async (ticket: Ticket) => {
    ...
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_DOMAIN}/tickets/${ticket.id}/comments`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_DOMAIN}/tickets/${selectedTicket.id}/comments`,
        { text: newComment },
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
  ...
  const handleDeleteTicket = async () => {
    ...
    try {
      await axios.delete(
        `${import.meta.env.VITE_API_DOMAIN}/tickets/${selectedTicket.id}`,
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
      );
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_DOMAIN}/tickets`,
        {
          subject: newSubject,
